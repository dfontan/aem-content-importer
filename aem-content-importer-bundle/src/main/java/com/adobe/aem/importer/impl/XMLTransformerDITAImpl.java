...
public class XMLTransformerDITAImpl extends AbstractXmlTransformer implements XMLTransformer  {
	...
	public void transform(Node srcPath, Properties properties) throws ImporterException {
		...
		// Source Path check
		if (properties==null)
			throw new ImporterException(AEM_IMPORTER_EXCEPTION_TYPE.ERROR_PARAMS, "Properties file cannot be NULL");
		...
		try {
			if (!srcPath.hasNode(masterFile))
				throw new ImporterException(AEM_IMPORTER_EXCEPTION_TYPE.ERROR_PARAMS, "Master File "+masterFile+" not available in the folder "+srcPath.getPath());
			...
			if (!session.itemExists(xslt))
				throw new ImporterException(AEM_IMPORTER_EXCEPTION_TYPE.ERROR_PARAMS, "XSLT Node File not available ("+xslt+")");
			final Node xsltNode = session.getNode(xslt);
			// Package Template Check
			if (!session.itemExists(packageTpl))
				throw new ImporterException(AEM_IMPORTER_EXCEPTION_TYPE.ERROR_PARAMS,"Package Template Node File not available ("+packageTpl+")");
			...
			if (e instanceof ImporterException) {
				...
				throw new ImporterException(AEM_IMPORTER_EXCEPTION_TYPE.UNEXPECTED, e.getMessage(), e);
			}
